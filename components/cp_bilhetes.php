...
if (!isset($_SESSION["id_user"])):
    ...
    if (isset($_GET["msg"])) {
        ...
        switch ($_GET["msg"]) {
            case 0:
                $message = "Compra efetuada com sucesso. Verifique aqui os seus bilhetes.";
                $class = "alert-success";
                break;
            default:
                $msg_show = false;
    ...
    $query = "SELECT eventos.nome, DATE_FORMAT(DATE(data_eventos.data), '%d!%m!%y'), TIME_FORMAT(TIME(data_eventos.data), '%H:%i'), id_bilhetes, bilhetes.nome, bilhetes.hash, eventos.preco_reserva, bilhetes.ativo
FROM bilhetes
LEFT JOIN compras
...
ORDER BY id_bilhetes  DESC;";
    ...
    if (mysqli_stmt_prepare($stmt, $query)) {
        ...
        mysqli_stmt_bind_result($stmt, $nome_evento, $data_evento, $hora_evento, $id_bilhetes, $nome_bilhetes, $bilhetes_hash, $preco, $ativo);
        mysqli_stmt_store_result($stmt);
        $bilhetes_n = mysqli_stmt_num_rows($stmt);

        if (mysqli_stmt_num_rows($stmt) == 0) {
            echo "<div class='text-center center-pagina'><h1 class='mb-50px'>Não tens bilhetes.</h1></div>";
        } else {
            ?>
                ...
                <div class="swiper-container mySwiper">
                    <div class="swiper-wrapper">
                        ...
                        while (mysqli_stmt_fetch($stmt)) {
                            ...
                            ?>
                            <div class="bilhetes swiper-slide"<?php if ($ativo == 0) {
                                echo "style='background-color: #9e9e9e;' ";
                            } ?>>
                                <div class="desc-bilhete container-fluid">
                                    <div class="row mb-4 mt-2 pb-2 align-items-center">
                                        <div class="col">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="120" height="auto"
                                                 fill="#1c1c1e"
		...
		c5.7-32.7,22.3-57.9,50.6-74.7c13.2-8,27.2-15.2,41.9-19.8c30.8-9.6,28.3-31.7,25.1-39.6C191.9,11.2,182.8,5.5,177.5,3.7z"/>
                                        ...
                                        <div class="col-auto text-end">
                                            <div class="text-uppercase small fw-600">
                                                Preço
                                            </div>
                                            <div class="text-uppercase h5 mb-0">
                                                <?php
                                                if (round($preco) == 0) {
                                                    echo "<small>Gratuito</small>";
                                                } else {
                                                    echo round($preco) . "€";
                                                }
                                                ?>
